...
object UsedByResolver {
    ...
    private fun recordFieldDependents(field: Field) {
                ...
                group.dependencies.forEach { dep ->
                    when (val source = dep.source) {
                        ...
                        is FeatureDependencyResolver.DependencySource.ThirdParty ->
                            source.value.mainToggleField?.javaField?.let { main ->
                                addReverse("${main.declaringClass.name}#${main.name}", entry)
                            }
                    }
                }
            }
        ...
        val tpAnn = field.getAnnotation(ThirdPartyDependency::class.java)
        val classAnn = field.declaringClass.getAnnotation(ThirdPartyDependency::class.java)
        val thirdParty = tpAnn?.value ?: classAnn?.value ?: return
        thirdParty.mainToggleField?.javaField?.let { main ->
            addReverse("${main.declaringClass.name}#${main.name}", entry)
        }
    }
