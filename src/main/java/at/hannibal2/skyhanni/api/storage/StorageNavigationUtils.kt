...
object StorageNavigationUtils {
    ...
    fun makeNavigatableChatList(results: List<StorageSearchResult>, allowServerChange: Boolean) {
        ...
        ChatUtils.chat("Found ${results.size} stacks (->${results.sumOf { it.item.count }} Items) matching the query. They were found in ${groupedResults.size} different locations:")
        ...
        groupedResults.forEach { (_, items) ->
            val first = items.first()
            ChatUtils.clickableChat(
                "§7- §f${first.getDisplayName()} ${if (first.page != null) " Page ${first.page}" else ""}: §b${items.sumOf { it.item.count }} items",
                {
